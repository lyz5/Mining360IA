    ...
    try:
        with sync_playwright() as p:
            ...
            for width,height,name in ((1440,900,"desktop"),(1280,800,"compact-desktop"),(1100,800,"laptop"),(768,1024,"tablet"),(390,844,"mobile")):
                ...
                page.wait_for_selector('[data-content]:visible'); checks=page.evaluate("""() => {const table=document.querySelector('.bcc-sales-table');return {overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+1,tableOverflow:table.scrollWidth>table.parentElement.clientWidth+1,hero:document.querySelector('[data-hero-value]')?.textContent,cards:document.querySelectorAll('[data-line]').length,salesRows:document.querySelectorAll('[data-sales-body] tr').length,technical:Boolean(document.querySelector('[data-bm-sync],[data-bm-confirm]'))}}""")
                if checks["overflow"] or checks["tableOverflow"] or checks["cards"] != 4 or checks["salesRows"] != 4 or checks["technical"] or "377.9M" not in checks["hero"]: raise AssertionError(f"{name}: {checks}")
