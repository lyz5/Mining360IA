...
class LegacyAIAdapter(ABC):
    ...
    def normalize_error(self, exception) -> LegacyAIError:
        if isinstance(exception, LegacyAIError):
            return exception
        if isinstance(exception, requests.Timeout):
            return LegacyAIError("TIMEOUT", str(exception), status_code=504)
        if isinstance(exception, requests.ConnectionError):
            return LegacyAIError("CONNECTION_ERROR", str(exception), status_code=503)
        ...
        if status in {401, 403}:
            return LegacyAIError("AUTHENTICATION_ERROR", message, status_code=503, retryable=False)
        if status == 429:
            return LegacyAIError("RATE_LIMIT", message, status_code=429)
        if status and status >= 500:
            return LegacyAIError("PROVIDER_UNAVAILABLE", message, status_code=503)
        if status == 404:
            return LegacyAIError("MODEL_UNAVAILABLE", message, status_code=503)
        if status and status >= 400:
            return LegacyAIError("INVALID_REQUEST", message, status_code=400, retryable=False)
        return LegacyAIError("UNKNOWN_PROVIDER_ERROR", message)
    ...
    def _test_request(model):
        from .legacy_ai_types import AIRequest
