    ...
    items.extend(
        {
            "id": item.id,
            "section": section.code,
            "entity_type": "metric" if item.entity_type == "KPI" else item.entity_type.lower().replace(" ", "_"),
            "canonical_value": item.canonical_term,
            "synonym_value": item.synonym,
            "normalized_value": item.normalized_value,
            "normalized_synonym_key": item.normalized_synonym_key,
        ...
        payload.append(
            {
                ...
                "kpi_targets": [
                ...
                ] + [
                    {
                        ...
                        "threshold_evaluation_rule": item.threshold_evaluation_rule,
                        "unit": item.unit,
                        "powerbi_measure_name": item.powerbi_measure_name,
                        "powerbi_measure_full_reference": item.powerbi_measure_full_reference,
                        "default_time_grain": item.default_time_grain,
                        "default_comparison_type": item.default_comparison_type,
                        "supported_dimensions": item.supported_dimensions,
                        "required_filters": item.required_filters,
                        "optional_filters": item.optional_filters,
                        "related_kpis": item.related_kpis,
                        "diagnostic_kpis": item.diagnostic_kpis,
                        "default_answer_template": item.default_answer_template,
                        "business_explanation_template": item.business_explanation_template,
                        "clarification_message": item.clarification_message,
                        "ai_usage_instructions": item.ai_usage_instructions,
                        "minimum_data_completeness": str(item.minimum_data_completeness or ""),
                        "minimum_equipment_count": item.minimum_equipment_count,
                        "freshness_requirement": item.freshness_requirement,
                        "data_quality_warning_message": item.data_quality_warning_message,
                    }
